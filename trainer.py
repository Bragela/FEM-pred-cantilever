...
def no_grad_loop(data_loader, model, png_cnt, epoch=2, device="cuda", batch_size = 64):
    ...
    cnt = 0
    for i, (forces, coords, coords_original, FEM_stress, FEM_disp) in enumerate(data_loader):
        ...
        with autocast(): 
            disp_pred, stress_pred = model(forces, coords)
...
def train(model: NeuralNet, num_epochs, batch_size, train_loader, test_loader, validation_loader, learning_rate=1e-3, device="cuda"):
    ...
    for epoch in range(num_epochs):
        ...
        for forces, coords, coords_original, FEM_stress, FEM_disp in loader:  
